...
class Problem:
    ...
    def preprocess(self):
        ...
        self.active_process = ppmc = subprocess.Popen(preprocessor, stdin=subprocess.PIPE, stdout=subprocess.PIPE,
    ...
    def decompose_nested_primal(self):
        num_vars, edges, adj = cnf2primal(self.formula.num_vars, self.formula.clauses, self.formula.var_clause_dict,
                                          True)
    ...
    def call_solver(self, type):
        ...
        logger.info(
            f"Call solver: {type} with #vars {self.formula.num_vars}, #clauses {len(self.formula.clauses)}, #projected {len(self.projected)}")
    ...
    def solve(self):
        ...
        if hasattr(self, 'graph'):  # is tree decomposition already given (i.e. by argbtw)
            cnf2primal(self.formula.num_vars, self.formula.clauses, self.formula.var_clause_dict,
                       True)
        else:
