...
class VertexCover(Problem):
    ...
    def candidate_extra_cols(self,node):
        introduce = [var2size(node,v) for v in node.num_vertices if node.needs_introduce(v)]
        ...
        if join:
            q += "{}".format(" + ".join(join))
            if len(join) > 1:
                children = [vc for c in node.children for vc in c.num_vertices if vc in node.num_vertices]
    ...
    def filter(self,node):
        ...
        check = []
        for c in node.num_vertices:
            if node.needs_introduce(c):
                nv = [v for v in self.edges[c] if v in node.num_vertices] + [c]
                if len(nv) > 1:
