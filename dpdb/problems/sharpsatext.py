...
class SharpSatExt(Problem):
    ...
    def candidate_extra_cols(self,node):
        return ["{} AS model_count".format(
                " * ".join(set([var2cnt(node,v) for v in node.num_vertices] +
                               [node2cnt(n) for n in node.children])) if node.num_vertices or node.children else "1"
                )]
    ...
    def after_solve_node(self, node, db):
        cols = [var2col(c) for c in node.num_vertices]
    ...
    def solve_sat(self, node, db, cols, vals):
        try:
            where = []
            orig_vars = self.abstr.orig_vertices(node.num_vertices)
            ...
            for i,v in enumerate(vals):
                if v != None:
                    ...
                    n = self.abstr.orig_vertex(node.num_vertices[i])
                    if v:
                        clauses.append([n])
