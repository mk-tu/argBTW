...
class Problem(object):
    ...
    def candidates_select(self,node):
        ...
        if any(node.needs_introduce(v) for v in node.num_vertices):
            q += "WITH introduce AS ({}) ".format(self.introduce(node))

        q += "SELECT {}".format(
                ",".join([var2tab_col(node, v) for v in node.num_vertices]),
                )

        extra_cols = self.candidate_extra_cols(node)
        if extra_cols:
            q += "{}{}".format(", " if node.num_vertices else "", ",".join(extra_cols))

        if node.num_vertices or node.children:
            q += " FROM {}".format(
                    ",".join(set(["{} {}".format(var2tab(node, v), var2tab_alias(node, v)) for v in node.num_vertices] +
    ...
    def setup(self):
        ...
        def create_tables_for_node(n, workers = {}):
            ...
            if self.candidate_store == "table":
                db.create_table(f"td_node_{n.id}_candidate", [self.td_node_column_def(c) for c in n.num_vertices] + self.td_node_extra_columns())
